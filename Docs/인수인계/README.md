...
| ③ `ui_smoke_cli` | 마지막 줄 `스모크 테스트 전부 통과`, 종료 코드 0 (20번 = 회전 기즈모, 21번 = 부드러운 도트 이동, 22번 = 캐릭터 이동, 23번 = 재생할 애니 분리) | ✓ (09-15. 09-14 에 `puppet_test/` **없는 상태**에서도 통과 = 새 clone 조건) |
| ④ `rootbone_check_cli` | `모든 파트의 루트 본 정상` (Hips = pelvis, Foot = foot) | ✓ (09-15) |
| ⑤ `drift_check_cli` | `포즈 고정 OK` | ✓ (09-15) |
| ⑥ `layer_check_cli` | `레이어 검사 전부 통과` — [0] 기본: 15파트, 발가락은 발에 합침, 발 늘이기 1.0 고정, 씬에 발가락 본 없음 · [1]~[4] 발가락 분리 모드: 17파트/15줄, z 가 항상 발+1, 구운 씬·rig.json 까지 | ✓ (09-15) |
...
2. **부드러운 도트 이동 — 게임 쪽 확인** — 찌글거림은 셰이더로 대응 완료(계단 재생은 사용자가 기각, [02 P5](02_버그와_교훈.md)). 단 효과는 **게임이 캐릭터를 확대해서 그릴 때만** 난다. 게임을 저해상도 화면에 1:1 로 그리고 화면 전체를 키우는 방식으로 만들면 효과가 없으니, 게임 렌더 방식이 정해지면 다시 볼 것. 뛰기처럼 빠른 동작에서 남는 변화는 3D 렌더와 같은 수준(동작 자체)이다.
