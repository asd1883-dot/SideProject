...
| ③ `ui_smoke_cli` | 마지막 줄 `스모크 테스트 전부 통과`, 종료 코드 0 (20번 = 회전 기즈모, 21번 = 부드러운 도트 이동, 22번 = 캐릭터 이동, 23번 = 재생할 애니 분리, 24번 = 레스트 자세 찾기 + 레스트 시점이 베이크까지, 25번 = 재생 속도·스크럽, 26번 = 리깅 애니메이션 세트 편집·프리셋, 27번 = 동작 평면화, 28번 = 레스트 자동, 29번 = 도트 아웃라인 전체 실루엣/파트별, 30번 = 추가 동작 폴더, 31번 = 바인드 포즈·지금 프레임을 레스트로, 32번 = 도트 격자 고정, 33번 = 상하체 합성 팝업, 34번 = 선 색 방식 카툰/픽셀 퍼펙트) | ✓ (09-21. 09-14 에 `puppet_test/` **없는 상태**에서도 통과 = 새 clone 조건) |
...
1. **장비(무기) 굽기 툴 — 두 번째 창**(09-21 사용자 발의 · 구조 합의 · 런타임 쪽만 완료, 01 §4-27) — 남은 것: 굽기 코어 → 창(모델·잡는 법·그리기 순서·미리보기·굽기) → 무기 안의 움직이는 파트. 아래는 그 전의 메모: 지금은 런타임 부착만 된다(검증에 쓴 헬멧 PNG 는 손으로 만든 것). 캐릭터와 같은 카메라(`rig.json` 의 `view` → `DRBaker.apply_view()`)·같은 레스트 포즈로 장비 메쉬를 구워 `DREquipItem`(오프셋 자동 계산 포함)을 만드는 흐름이 필요하다.
2. ~~게임 쪽 상태별 씬 교체~~ — **09-21 완료**: 런타임 `DRPuppetSet` 이 `sets.json` 을 읽어 세트를 하나로 묶는다(`play("이름")` · 발 자리 맞춤 · 마우스 조준 · 좌우 반전, [01 §4-26](01_설계와_구조.md)). 시험장 `res://test/puppet_test.tscn`(F6). 남은 것: 세트가 바뀔 때 장비 자동 재장착 · 도트 격자 고정과 같이 쓰기 · 앉기 진입 같은 연결 동작.
...
10. **게임 구조(09-17~18 사용자와 합의, 아직 코드 없음)** — 1941 군인 옆시점(좌우만), 조준 ±20°, 걸으며 쏘기는 병사별 on/off 규칙, 피격 시 팔 떨어짐, 앉기·포복.
    - **병사 씬(상태를 가짐) ≠ 퍼펫(보이는 껍데기)** — 떨어진 팔·장비는 병사가 기억하고 퍼펫에 적용. 구운 `puppet.tscn` 은 손대지 않고 병사 씬 안에 인스턴스로.
    - **자세 계열 = 세트 = 리그 씬**(서기/앉기/포복). 같은 계열 동작은 **세트 하나에**(사용자가 Idle·Walk·Sprint 를 세트 3개로 따로 구운 적 있음 — 그러면 AnimationPlayer 가 셋으로 갈린다).
    - **09-21: 상하체 합성은 도구에 들어갔다**(5번 `상하체 합성…` 팝업 → 하체 애니 + 상체 애니 = 새 동작, [01 §4-23](01_설계와_구조.md)). 게임은 합성된 이름을 그냥 재생하면 된다.
    - **★ 결정(09-21, 사용자): 굽기 전에 합치는 지금 방식으로 간다.** 실행 중에 상하체를 따로 재생하는 방식(아래 AnimationTree)은 안 쓴다 — "어느 자세는 어느 보간으로 보정해야 하는지를 런타임에 관리하는 게 너무 번거롭다". Flash 시절 Warfare 1944 류와 같은 구조: 상태마다 미리 만든 동작 하나를 고른다. 자세 3 × 이동 2 × 상체 동작 4 = 24개쯤은 감당할 만하다. 실행 중 계산은 조준 각도(몸통·팔 뼈를 몇 도 돌리기)·반동·피격 움찔·떨어진 팔(물리)에만 쓴다. 아래 AnimationTree 방식은 조합이 너무 많아지거나 전환 때 다리가 튀는 게 거슬릴 때의 대안으로만 남긴다(구운 동작·파트 이름은 그대로 쓸 수 있다).
    - **상하체 분리** — 리그마다 AnimationTree(Blend2 + 트랙 필터): 하체 = 이동, 상체 = 무기. 조준·사격·장전을 이동과 **같은 세트에** 구워야 하고 **그리기 순서는 수동 고정**(자동이면 애니마다 z 트랙이 달라 섞을 때 충돌). 포복은 분리 안 함.
    - 조준 ±20° = 애니 적용 뒤 몸통 본에 각도 더하기(머리·팔·총이 몸통 자식이라 한 덩어리로 돌아 왼손이 안 미끄러짐). 뻣뻣하면 오른손 기준 2본 IK — 그 세트는 동작 평면화로.
    - 고정할 이름 3가지: 파트 이름 · 동작 이름 · 세트 이름(게임 쪽이 이름으로 참조).
